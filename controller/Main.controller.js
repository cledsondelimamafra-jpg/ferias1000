            ...
            fetch(sUrlCidade, { headers: { 'Accept': 'application/json' } })
                ...
                .then(function (data) {
                    if (data && data.length > 0) {
                        ...
                        // Extrai o nome limpo antes da vírgula para não quebrar a API de turismo
                ...
                .then(function (pontosData) {
                    if (pontosData && pontosData.length > 0) {
                        ...
                        // Plano de contingência: Garante que a lista mude com o destino falado
                        var sNomeLocal = oModel.getProperty("/local/cidade").split(',')[0];
                        oModel.setProperty("/lugares", [
                            { nome: "Tour Histórico por " + sNomeLocal, categoria: "Passeio Guiado" },
                            { nome: "Mirante Central de " + sNomeLocal, categoria: "Vista Panorâmica" },
                            { nome: "Mercado Municipal de " + sNomeLocal, categoria: "Gastronomia Local" }
                        ]);
                    }
                })
